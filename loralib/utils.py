...
def get_lora_xs_parameters(model, bias='none'):
    params = []
    for name, param in model.named_parameters():
        if bias == 'none':
            if '_R' in name and param.requires_grad:
                params.append(param)
        elif bias == 'all':
            if (('_R' in name) or ('bias' in name)) and param.requires_grad:
                params.append(param)
        elif bias == 'lora_only':
            if '_R' in name and param.requires_grad:
                params.append(param)
                # Optionally, also add an associated bias if desired:
                bias_name = name.replace('_R', 'bias')
                if bias_name in dict(model.named_parameters()):
                    params.append(dict(model.named_parameters())[bias_name])
        else:
            raise NotImplementedError
    return params
